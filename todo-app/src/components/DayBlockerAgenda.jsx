...
export function DayBlockerAgenda({
  ...
  const positionedTasks = useMemo(() => {
    return scheduledTasks.map((task, idx) => {
      ...
      const overlapsPrev = idx > 0 && task.startMinutes < scheduledTasks[idx - 1].endMinutes;
      const overlapsNext = idx < scheduledTasks.length - 1 && task.endMinutes > scheduledTasks[idx + 1].startMinutes;
  ...
  const freeGaps = useMemo(() => {
    ...
    for (let i = 0; i < scheduledTasks.length - 1; i++) {
      ...
      if (gapMins >= 30) {
        const top = ((current.endMinutes - START_HOUR * 60) / 60) * HOUR_HEIGHT + 8;
        const sh = Math.floor(current.endMinutes / 60);
        const sm = current.endMinutes % 60;
        gaps.push({
          startMin: current.endMinutes,
          gapMins,
          top,
          suggestedTime: `${String(sh).padStart(2, '0')}:${String(sm).padStart(2, '0')}`,
        });
      }
    }
    return gaps;
  ...
  return (
    <div className="relative w-full bg-surface-glass backdrop-blur-xl border border-border-glass rounded-3xl p-4 sm:p-6 shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-mono-label text-on-surface-variant font-mono text-xs">
              07:00 – 23:00 Chrono Canvas • {scheduledTasks.length} {scheduledTasks.length === 1 ? 'task booked' : 'tasks booked'}
            </p>
          </div>
        </div>

        <button
          onClick={() => onNewTask({ type: 'specific-day', dueTime: '10:00', duration: 45 })}
          className="px-3.5 py-1.5 bg-primary/15 text-primary hover:bg-primary/25 border border-primary/30 rounded-xl text-label-md font-label-md font-bold transition-all flex items-center gap-1.5 cursor-pointer"
        >
      ...
      <div className="relative w-full overflow-x-hidden overflow-y-auto" style={{ minHeight: '650px' }}>
        <div className="relative w-full" style={{ height: `${totalGridHeight}px` }}>
          ...
          {Array.from({ length: END_HOUR - START_HOUR + 1 }).map((_, idx) => {
            ...
            return (
              ...
              >
                {/* Time Axis Column */}
                <div className="w-16 sm:w-20 shrink-0 text-mono-label font-mono text-xs text-on-surface-variant/70 font-semibold pt-1 select-none">
                  {hourFormatted}
                </div>

                {/* Horizontal Grid Wire */}
                <div
                  onClick={() => onNewTask({ type: 'specific-day', dueTime: `${String(hour).padStart(2, '0')}:00`, duration: 45 })}
          ...
          {freeGaps.map((gap, i) => {
            ...
            const gapLabel = hours > 0 ? `${hours}h${mins > 0 ? ` ${mins}m` : ''}` : `${mins}m`;

            return (
              <div
                key={i}
                style={{ top: `${gap.top}px`, left: '84px' }}
                className="absolute z-10 pointer-events-auto"
              >
                <button
                  onClick={() => onNewTask({ type: 'specific-day', dueTime: gap.suggestedTime, duration: Math.min(gap.gapMins, 60) })}
                  className="px-2.5 py-0.5 rounded-full bg-surface-container-high/80 hover:bg-primary/20 border border-border-glass hover:border-primary/40 text-on-surface-variant hover:text-primary transition-all text-mono-label font-mono text-[10px] flex items-center gap-1 backdrop-blur-md cursor-pointer shadow-sm group"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/80 group-hover:animate-ping" />
                  <span>{gapLabel} Free Window</span>
                  <span className="opacity-0 group-hover:opacity-100 transition-opacity font-bold">+ Schedule</span>
                </button>
              </div>
            );
          })}
          ...
          {nowOffset !== null && (
            ...
            >
              {/* Pulsing "NOW" Badge */}
              <div className="w-16 sm:w-20 shrink-0 flex items-center gap-1.5 justify-start">
          ...
          {positionedTasks.map(task => {
            ...
            // Width and left positioning for overlapping blocks
            const leftCalc = task.totalCols > 1
              ? `calc(72px + ${task.colIndex * 48}%)`
              : '76px';
            const widthCalc = task.totalCols > 1
              ? '48%'
              : 'calc(100% - 84px)';
            ...
            if (isHigh) {
              borderStyle = 'border-error/40 shadow-[0_0_15px_rgba(244,63,94,0.12)]';
              bgStyle = 'bg-error/10 hover:bg-error/15';
              accentBadge = 'bg-error/20 text-error';
            } else if (isMedium) {
              borderStyle = 'border-tertiary/40 shadow-[0_0_15px_rgba(245,158,11,0.12)]';
            ...
            const endTimeFormatted = formatDisplayTime(
              `${Math.floor(task.endMinutes / 60)}:${String(task.endMinutes % 60).padStart(2, '0')}`
            );
            ...
            return (
              <div
                ...
                className={`absolute z-10 rounded-2xl border p-3 flex flex-col justify-between backdrop-blur-xl transition-all duration-200 group ${borderStyle} ${bgStyle}`}
              >
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2.5 min-w-0">
                    ...
                    <div className="min-w-0">
                      <span className={`text-label-md font-bold block truncate text-on-surface ${isCompleted ? 'line-through opacity-70' : ''}`}>
                        {task.title}
                      </span>
                      <span className="text-mono-label font-mono text-[11px] text-on-surface-variant font-medium flex items-center gap-1">
                        <span className="material-symbols-outlined" style={{ fontSize: '13px' }}>schedule</span>
                        {startTimeFormatted} – {endTimeFormatted}
                        <span className="opacity-60 font-sans">({task.duration}m)</span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    {task.hasConflict && (
                      <span className="px-1.5 py-0.5 rounded-md bg-amber-500/20 text-amber-400 text-[10px] font-mono font-bold flex items-center gap-0.5 border border-amber-500/30" title="Time overlap with another task">
                    ...
                    {accentBadge && (
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-mono uppercase font-bold ${accentBadge}`}>
                        {task.priority}
                      </span>
                    )}
                    ...
                    <div className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity flex items-center gap-0.5">
                      <button
                        ...
                        className="p-1 text-on-surface-variant hover:text-primary rounded hover:bg-primary/10 transition-colors"
                        title="Edit task"
                      >
                ...
                {(task.category || task.notes) && (
                  <div className="flex items-center gap-2 mt-1 pt-1 border-t border-border-glass/40 text-[11px] text-on-surface-variant truncate">
                    {task.category && (
                      <span className="px-1.5 py-0.2 rounded bg-surface-container-highest font-mono text-[10px] text-on-surface">
                        #{task.category}
                      </span>
                    )}
                    {task.notes && (
